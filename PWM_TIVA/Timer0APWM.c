...
/*
#define PWM_ENABLE_PWM3EN       0x00000008  // PWM1 Output Enable
...
*/
void PWM3A_Init(uint16_t period, uint16_t duty){
SYSCTL_RCGCPWM_R |= 0x00000001; // 1) activate clock for PWM1
// allow time to finish activating
while((SYSCTL_PRPWM_R&0x00000001)==0){};
SYSCTL_RCGCGPIO_R |= 0x00000002; // activate clock for Port B
// allow time to finish activating
while((SYSCTL_PRGPIO_R&0x00000002)==0){};
GPIO_PORTB_AFSEL_R |= 0x20; // 2) enable alt funct on PB5
GPIO_PORTB_ODR_R &= ~0x20; // disable open drain on PB5
GPIO_PORTB_DEN_R |= 0x20; // enable digital I/O on PB5
GPIO_PORTB_AMSEL_R &= 0x20; // disable analog function on PB5
// configure PB6 as PWM
GPIO_PORTB_PCTL_R = (GPIO_PORTB_PCTL_R&0xFF0FFFFF)+0x00400000;
SYSCTL_RCC_R = 0x00100000 | // 3) use PWM divider
((SYSCTL_RCC_R & (~0x000E0000)) + // clear PWM divider field
	0x00000000); // configure for /2 divider
PWM0_1_CTL_R = 0; // 4) re-loading down-counting mode
// PB6 goes low on LOAD
PWM0_1_GENB_R = 0x00000C08; // PB5 goes high on CMPA down
PWM0_1_LOAD_R = period - 1; // 5) cycles needed to count down to 0
PWM0_1_CMPB_R = duty - 1; // 6) count value when output rises
PWM0_1_CTL_R |= 0x00000001; // 7) start PWM0 Generator 1
PWM0_ENABLE_R |= 0x00000008; // enable PWM0 Generator 1
